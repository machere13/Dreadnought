...
`ButtonCore` — тип результата вычисления состояния в первом слое. `ButtonAdapter` создаёт нативный `<button>` с тем же поведением, но без визуального варианта. `useButton` возвращает свойства для собственной разметки на нативном `<button>`.
