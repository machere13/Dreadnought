...
`Button.module.css` использует токены для настраиваемых значений: цветов, размеров, отступов, границ, тени, состояний, иконки и индикатора загрузки. Шрифтовой класс `.dreadnought-text-button` тоже читает токены темы — `font-family`, размер, насыщенность, высоту строки, межбуквенный интервал, начертание и регистр. Внутри репозитория общие значения лежат в `packages/themes/src/default/tokens/global/`, значения Button — в `tokens/components/Controls/Button/`; токены разбиты на файлы по назначению. Шрифтовой класс находится отдельно в `components/Controls/Button/typography.css`. При использовании готового компонента тема подключается автоматически; `@dreadnought/themes/default.css` нужен только при самостоятельном подключении темы без готовой точки входа. Стили Button и его шрифтовой класс находятся в `@layer dreadnought`: переданный через `className` пользовательский CSS без слоя может перекрыть отдельное свойство без `!important`. Для повторяемой настройки темы используйте токены. Если требуется другой порядок элементов или новая разметка, используйте `ButtonAdapter` со своим CSS Module.
